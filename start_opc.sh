...
--publish $opc_port:$opc_port `#port for opcua server` \
...
--showpnjsonph                `#show OPC Publisher configuration file on log, also available on the publish port` \
--addtrustedcertfile=$clt_crt `#add the client certificated to trusted certificates` \
--loglevel=debug              `#fatal, error, warn, info, debug, verbose` \
--portnum=$opc_port           `#the server port of the OPC server endpoint` \
--webport=8080                `#port to serve http://....:8080/pn.json` \
